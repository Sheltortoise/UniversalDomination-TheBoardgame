...
public class GameBoardBattlefield extends AppCompatActivity {
    ...
    private ArrayList<Player> playerList = new ArrayList<Player>();
        private Player animusPlayer = new Player("", 0, 0, null, 100, 0);
        private Player antibytePlayer = new Player("", 0, 0, null, 100, 0);
        private Player elementalPlayer = new Player("", 0, 0, null, 100, 0);
        private Player grotianPlayer = new Player("", 0, 0, null, 100, 0);
        private Player hivePlayer = new Player("", 0, 0, null, 100, 0);
        private Player humanityPlayer = new Player("", 0, 0, null, 100, 0);

    private ArrayList<ResourceMound> resourceMoundList = new ArrayList<ResourceMound>();
    private ArrayList<ImageView> allUnits = new ArrayList<ImageView>();
    ...
    public void reinforcementPhase() {
        ...
        // Determine which player has control.
        for(int i = 0; i < playerList.size(); i++) {
            if(playerList.get(i).isTurn == turnCounter) {
                playerList.get(i).resources += 5;
                messageBoardText.setText("Your Resources: " + playerList.get(i).resources);
    ...
    public void movementPhase() {
        ...
        // Ensures Resource Cap
        for(int i = 0; i < playerList.size(); i++) {
            if(playerList.get(i).resources > 20) {
                playerList.get(i).resources = 20;
            }
        }
    }
    ...
    public void checkWinCondition() {
        ...
        for(int i = 0; i < playerList.size(); i++) {
            for(int j = 0; j < playerList.get(i).unitList.size(); j++) {
                if(playerList.get(i).unitList.size() == 0) {
    ...
    public void startAction(View view) {
        ...
        if(view == centerBtn && isMovement) {
            moveUnit("waste");
        }
        else if(view == upBtn && isMovement) {
            moveUnit("up");
        }
        else if(view == rightBtn && isMovement) {
            moveUnit("right");
        }
        else if(view == downBtn && isMovement) {
            moveUnit("down");
        }
        else if(view == leftBtn && isMovement) {
            moveUnit("left");
        }
        /*
        else if(view == topRightBtn && isMovement) {
            moveUnit("");
        }
        else if(view == bottomRightBtn && isMovement) {
            moveUnit("");
        }
        else if(view == bottomLeftBtn && isMovement) {
            moveUnit("");
        }
        else if(view == topLeftBtn && isMovement) {
            moveUnit("");
        }
        */
        ...
        // During Combat Phase
        /*
        if(view == upBtn && isCombat) {
            attackPlot("up");
        }
        else if(view == rightBtn && isCombat) {
            attackPlot("right");
        }
        else if(view == downBtn && isCombat) {
            attackPlot("down");
        }
        else if(view == leftBtn && isCombat) {
            attackPlot("left");
        }
        else if(view == topRightBtn && isCombat) {
            attackPlot("");
        }
        else if(view == bottomRightBtn && isCombat) {
            attackPlot("");
        }
        else if(view == bottomLeftBtn && isCombat) {
            attackPlot("");
        }
        else if(view == topLeftBtn && isCombat) {
            attackPlot("");
        }
        else if(view == centerBtn && isCombat) {
            attackPlot("waste");
        }
        */
    }
    public void moveUnit(String direction) {
        for(int i = 0; i < playerList.size(); i++) {
            ...
            if(tempPlayer.isTurn == turnCounter) {
                //for(int j = 0; j < tempPlayer.unitList.size(); j++) {
                    Unit tempUnit = tempPlayer.unitList.get(1);

                    int tempCellPos = 0;

                    if(tempUnit.mobility != 0) {
                        checkMove(tempPlayer, tempUnit, direction, tempCellPos);
    ...
    public void checkMove(Player player, Unit unit, String direction, int cellPos) {
        String currentPos = unit.position;
        ...
        if(direction.equals("waste")) {
            if(player == animusPlayer) {
                animusPlayer.unitList.get(cellPos).mobility = 0;
            }
            else if(player == antibytePlayer) {
                antibytePlayer.unitList.get(cellPos).mobility = 0;
            }
            else if(player == elementalPlayer) {
                elementalPlayer.unitList.get(cellPos).mobility = 0;
            }
            else if(player == grotianPlayer) {
                grotianPlayer.unitList.get(cellPos).mobility = 0;
            }
            else if(player == hivePlayer) {
                hivePlayer.unitList.get(cellPos).mobility = 0;
            }
            else if(player == humanityPlayer) {
                humanityPlayer.unitList.get(cellPos).mobility = 0;
            }
        }
        else if(direction.equals("up") && currentY != 0) {
            currentY -= 1;
        }
        else if(direction.equals("right") && currentX != 29) {
            currentX += 1;
        }
        else if(direction.equals("down") && currentY != 19) {
            currentY += 1;
        }
        else if(direction.equals("left") && currentX != 0) {
            currentX -= 1;
        }
        ...
        for(int i = 0; i < playerList.size(); i++) {
            ...
            for(int j = 0; j < tempPlayer.unitList.size(); j++) {
                ...
                if(tempUnit.position == newPos && tempUnit.species != unit.species) {
                    // Do Nothing, because the enemy unit is in the way
                }
                else {
                    makeMove(player, unit, cellPos, currentPos, newPos);
                }
            }
        }
    }
    public void makeMove(Player player, Unit unit, int cellPos, String currentPos, String newPos) {
        float[] tempArr = checkOccupancy(newPos, unit.points);

        if(tempArr[0] != 100) {
            for(int i = 0; i < allUnits.size(); i++) {
                float tempX = allUnits.get(i).getX();
                float tempY = allUnits.get(i).getY();

                String tempPosCode = positionToPosCode(tempX, tempY);

                ImageView tempUnitView = allUnits.get(i);

                if(unit.position == tempPosCode) {
                    if(player == animusPlayer) {
                        animusPlayer.unitList.get(cellPos).mobility -= 1;
                        animusPlayer.unitList.get(cellPos).position = newPos;
                    }
                    else if(player == antibytePlayer) {
                        antibytePlayer.unitList.get(cellPos).mobility -= 1;
                        antibytePlayer.unitList.get(cellPos).position = newPos;
                    }
                    else if(player == elementalPlayer) {
                        elementalPlayer.unitList.get(cellPos).mobility -= 1;
                        elementalPlayer.unitList.get(cellPos).position = newPos;
                    }
                    else if(player == grotianPlayer) {
                        grotianPlayer.unitList.get(cellPos).mobility -= 1;
                        grotianPlayer.unitList.get(cellPos).position = newPos;
                    }
                    else if(player == hivePlayer) {
                        hivePlayer.unitList.get(cellPos).mobility -= 1;
                        hivePlayer.unitList.get(cellPos).position = newPos;
                    }
                    else if(player == humanityPlayer) {
                        humanityPlayer.unitList.get(cellPos).mobility -= 1;
                        humanityPlayer.unitList.get(cellPos).position = newPos;
                    }
                    tempUnitView.setX(tempArr[0]);
                    tempUnitView.setY(tempArr[1]);
                }
            }
            plotRearranger(currentPos, unit.points);
        }
    }
    ...
    public void removeUnit(Unit unit) {
        ...
        for(int i = 0; i < playerList.size(); i++) {
            ...
            for(int j = 0; j < tempPlayer.unitList.size(); j++) {
                ...
                for(int z = 0; z < allUnits.size(); z++) {
                    float tempX = allUnits.get(z).getX();
                    float tempY = allUnits.get(z).getY();

                    String tempPosCode = tempX + "-" + tempY;

                    if(unit.position.equals(tempUnit.position) && unit.position.equals(tempPosCode)) {
                        ImageView tempUnitView = allUnits.get(z);

                        battlefield.removeView(tempUnitView);

                        allUnits.remove(z);
                        playerList.get(i).unitList.remove(j);

                        plotRearranger(unit.position, unit.points);
                    }
                }
            }
        }
    ...
    public void addUnit(View view) {
        ...
        if(view == baseCard) {
            ...
            if(animusPlayer.isTurn == turnCounter && isReinforcement) {
                ...
                if(inflateUnit(tempUnit, tempPlayer)) {
    ...
    public boolean inflateUnit(Unit unit, Player player) {
    ...
    public void plotRearranger(String posCode, int pts) {
        int[] converted = posCodeToInts(posCode);
        ...
        for(int i = 0; i < playerList.size(); i++) {
            ...
            for(int j = 0; j < tempPlayer.unitList.size(); j++) {
                ...
                for(int z = 0; z < allUnits.size(); z++) {
                    ...
                    float tempXPos = allUnits.get(z).getX();
                    float tempYPos = allUnits.get(z).getY();

                    String tempPosCode = tempXPos + "-" + tempYPos;

                    if(tempUnit.position == tempPosCode) {
                        // Moving a Large Unit
                        if (pts == 4 && tempPlot.points == 0) {
                            tempX = tempPlot.xPos1;
                            tempY = tempPlot.yPos1;
                        }
                        // Moving a Medium Unit
                        if (pts == 2 && tempPlot.points == 0) {
                            tempX = tempPlot.xPos1;
                            tempY = tempPlot.yPos1;
                        } else if(pts == 2 && tempPlot.points == 1 || pts == 2 && tempPlot.points == 2) {
                        ...
                        updatePlotsPlus(tempUnit.position, tempUnit.points);

                        tempUnitView.setX(tempX);
                        tempUnitView.setY(tempY);
                    }
                }
            }
        }
    }
    public String positionToPosCode(float posX, float posY) {
        String posCode = "";

        for(int i = 0; i < 30; i++) {
            for(int j = 0; j < 20; j++) {
                Plot tempPlot = vm.getVirtualPlots()[i][j];

                if(tempPlot.xPos1 == posX && tempPlot.yPos1 == posY) {
                    posCode = tempPlot.name;
                }
                else if(tempPlot.xPos2 == posX && tempPlot.yPos2 == posY) {
                    posCode = tempPlot.name;
                }
                else if(tempPlot.xPos3 == posX && tempPlot.yPos3 == posY) {
                    posCode = tempPlot.name;
                }
                else if(tempPlot.xPos4 == posX && tempPlot.yPos4 == posY) {
                    posCode = tempPlot.name;
                }
                else {
                    posCode = "error";
